...
//    	System.out.println(CloudSim.clock()+":"+CloudSim.getEntityName(ev.getSource())+"-> FogScheduler = "+ev.getTag());
        switch (ev.getTag()) {
            ...
            case WorkflowSimTags.CLOUDLET_UPDATE:
				...
				case ANT:
					if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
						processCloudletUpdateForANTInit(ev);
	            	}else if(WorkflowEngine.startlastSchedule==0){
	            		processCloudletUpdateForANTUpdate(ev);
					}else {
						processCloudletUpdateForANTGbest(ev);
					}
				  break;					
				case MINMIN:
				case MAXMIN:
...
//            System.out.println(cloudlet.getCloudletId()+".submit delay : "+delay);
    ...
    protected void processCloudletUpdateForANTInit(SimEvent ev) {
    	List<Cloudlet> cloudletList=getCloudletList();
    	List<CondorVM> vmList=getVmsCreatedList();
    	if(AntColonyOptimization.initFlag==0) {
    		startTime = System.currentTimeMillis();
    		WorkflowEngine engine = (WorkflowEngine)CloudSim.getEntity(workflowEngineId);
    		AntColonyOptimization antColonyOptimization = new AntColonyOptimization(0);
			antColonyOptimization.clearTrails();
    	}
    	List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
    	List<int[]> schedules=AntColonyOptimization.schedules;
    	for(int i=0;i<cloudletList.size();i++) {
    		int cloudletId=cloudletList.get(i).getCloudletId();
    		int vmId=schedules.get(count)[cloudletId];
    		cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForANTUpdate(SimEvent ev) {
    	List<Cloudlet> cloudletList=getCloudletList();
    	List<CondorVM> vmList=getVmsCreatedList();
    	if(WorkflowEngine.updateFlag2==1&&cloudletList.size()!=0) {
    	AntColonyOptimization antColonyOptimization = new AntColonyOptimization(workflowEngineId);      //LATER ADDED
			antColonyOptimization.updateTrails();
    	}
    	List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
    	List<int[]> newSchedules=AntColonyOptimization.newSchedules;
    	for(int i=0;i<cloudletList.size();i++) {
    		int cloudletId=cloudletList.get(i).getCloudletId();
    		int vmId=newSchedules.get(count2)[cloudletId];
    		cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForANTGbest(SimEvent ev) {
    	List<Cloudlet> cloudletList=getCloudletList();
    	List<CondorVM> vmList=getVmsCreatedList();
    	List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
    	for(int i=0;i<cloudletList.size();i++) {
    		int cloudletId=cloudletList.get(i).getCloudletId();
    		int vmId=AntColonyOptimization.gbest_schedule[cloudletId];
    		cloudletList.get(i).setVmId(vmId);
    		//setVmState(vmId);
    		scheduledList.add(cloudletList.get(i));
    	}
