...
//    	sendNow(getcontrollerId(), FogEvents.STOP_SIMULATION,null);
    ...
    protected void processJobReturnForANT(SimEvent ev) throws Exception {
    	Job job = (Job) ev.getData();
        if (job.getCloudletStatus() == Cloudlet.FAILED) {
            // Reclusteringengine will add retry job to jobList
        	System.out.println("failed");
        ...
        if(getJobsList().isEmpty() && jobsSubmitted == 0) {
    		//System.out.println("-------------------------------------------");
        	if(index1 != AntColonyOptimization.c) {
        		already = 1;
    			//处理完了一个粒子（初始化得到的粒子 ）
             	FogBroker.count++;
             	fitness[index1++] = caculatefitness();
        	...
        	else if(indexForUpdate != AntColonyOptimization.c) {
 	        	//处理完了所有的初始化得到的粒子,并且处理完了一个粒子（update后得到的粒子 ）
        		int gbestIndex = 0;
    			for(int i = 0; i < index1; i++) {
    				if(fitness[i] < AntColonyOptimization.gbest_fitness) {
    					AntColonyOptimization.gbest_fitness = fitness[i];
    					 gbestIndex = i;
    				}
    			}
    			AntColonyOptimization.gbest_schedule = AntColonyOptimization.pbest_schedule.get(gbestIndex);//更新全局最优的调度方案
    			
    			FogBroker.count2++;
	        	fitness2[indexForUpdate++] = caculatefitness();
	        	if(FogBroker.count2 != AntColonyOptimization.c && AntColonyOptimization.c > iterateNum) {
	        		init();
	        		sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
	        	}
 		    }
            if(index1 == AntColonyOptimization.c && indexForUpdate == AntColonyOptimization.c) {
 	        	//处理完了所有的初始化得到的粒子、update后得到的粒子
            	if(AntColonyOptimization.iterateNum > iterateNum) {
            		for(int i = 0; i < AntColonyOptimization.c; i++) { 
            			//更新个体最优
            			if(fitness[i] > fitness2[i]) {
            				int schedule1[] = AntColonyOptimization.pbest_schedule.get(i);
            				int schedule2[] = AntColonyOptimization.newSchedules.get(i);
            				for(int j = 0; j < schedule1.length; j++)
            					schedule1[j] = schedule2[j];
            				AntColonyOptimization.pbest_fitness[i] = fitness2[i]; 
            			}
            			else
            				AntColonyOptimization.pbest_fitness[i]=fitness[i];
	        		}
            		fitness = AntColonyOptimization.pbest_fitness;
            		for(int i = 0; i < AntColonyOptimization.c; i++) {  //更新全局最优
            			if(AntColonyOptimization.pbest_fitness[i] < AntColonyOptimization.gbest_fitness) {
            				//index1=i;
            				AntColonyOptimization.gbest_fitness = AntColonyOptimization.pbest_fitness[i];
            				AntColonyOptimization.gbest_schedule = AntColonyOptimization.pbest_schedule.get(i);
            			}
            		}
            		iterateNum++;
            		System.out.println("After "+iterateNum+" iterations:");
            		System.out.println("======gbest_fitness:========"+AntColonyOptimization.gbest_fitness);
            		updatebest.add(AntColonyOptimization.gbest_fitness);
//	              	printindicators(PsoScheduling.gbest_fitness);
            		...
            		if(AntColonyOptimization.iterateNum != iterateNum) {
        	        	...
        	        	init(); 
         		    	updateFlag2 = 1;
         		    	AntColonyOptimization.newSchedules.removeAll(AntColonyOptimization.newSchedules);
         		    	sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
    	        	}
            	}
            }
            ...
            if(AntColonyOptimization.iterateNum == iterateNum && startlastSchedule == 0) {
            	for(int i = 0; i < AntColonyOptimization.c; i++) {
            		if(AntColonyOptimization.pbest_fitness[i] == AntColonyOptimization.gbest_fitness) {
            			AntColonyOptimization.gbest_schedule = AntColonyOptimization.pbest_schedule.get(i);
            		}
